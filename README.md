...
- For Windows plugins: yabridge, built from the **`new-wine10-embedding`** branch. The released
  5.1.1 with Wine 9.22 or newer leaves plugin GUIs unable to take mouse input except at the top-left
  of the screen — [upstream PR #462](https://github.com/robbert-vdh/yabridge/pull/462)
- zlib, Xlib, a C++17 compiler

**On Arch derivatives, build yabridge with generic flags.** `-march=native` from `/etc/makepkg.conf`
miscompiles it, and every Windows VST3 then dies with an identical
`err:virtual:virtual_setup_exception stack overflow` —
[yabridge #449](https://github.com/robbert-vdh/yabridge/issues/449).
...
- **Two plugins take no mouse input** — Smooth Operator Pro (VST2) and Accentize SpectralBalance2
  (VST3), while soothe2, smartEQ4, smartComp3 and ERA6 are fine. Thirteen hypotheses were measured
  and refuted, and **Carla reproduces it**, so it is not this bridge.
